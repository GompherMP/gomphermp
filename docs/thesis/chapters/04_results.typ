...
Para sostener estas ejecuciones, la arquitectura interna del runtime se aleja de un modelo de creación dinámica convencional que introduciría un alto overhead. En su lugar, el sistema se apoya en dos subsistemas principales: un Gestor de Pool de Goroutines con hilos persistentes para manejar directivas estructuradas de forma óptima; y un Planificador de Tareas asíncrono que da soporte al paralelismo no estructurado. Este planificador construye un Grafo Acíclico Dirigido (DAG) en tiempo de ejecución para resolver y garantizar de forma segura las dependencias antes de la ejecución de cada tarea.

A modo de ilustración, la Figura 3 presenta un diagrama del funcionamiento del gestor de pool de Goroutines, y la Figura 4 presenta un ejemplo de un grafo acíclico dirigido a ser coordinado por el planificador de tareas.
