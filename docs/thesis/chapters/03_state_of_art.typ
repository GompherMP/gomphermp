...
El objetivo de esta revisión sistemática es realizar un análisis exhaustivo y crítico del estado del arte respecto a la implementación de modelos de paralelismo basados en directivas en lenguajes de propósito general modernos. El propósito principal es examinar cómo la literatura aborda la divergencia semántica entre las abstracciones de concurrencia nativas a los lenguajes modernos y el paralelismo tradicional. Con ello, se busca comprender las estrategias de diseño, los retos de compilación _source-to-source_ y los métodos de evaluación empleados para cerrar esta brecha, evitando así el denominado 'problema de los dos lenguajes' y la pérdida de los beneficios que ofrecen dichos lenguajes modernos.
...
- Examinar las propuestas de implementación existentes, con un enfoque en las arquitecturas de compilador (_source-to-source_) y librerías de runtime, y sintetizar los retos (sintácticos, semánticos y de rendimiento) reportados en la literatura.

- Identificar las metodologías de evaluación (métricas y _benchmarks_) utilizadas para validar estas implementaciones y consolidar los beneficios reportados en términos de rendimiento, productividad y mantenibilidad del código.
...
- *Enfoque de la Investigación:* Se priorizará investigaciones que detallan la arquitectura de compiladores (especialmente _source-to-source_), librerías de runtime, metodologías de implementación, o herramientas para el análisis de rendimiento de dichos paradigmas.
...
Aunque no fue recuperado por las cadenas automáticas iniciales, se incluye el trabajo de Madridejos Zamorano (2015), localizado mediante búsqueda manual en el Archivo Digital UPM. Se trata de literatura gris académica pertinente y de alta relevancia, al documentar un intento pionero por integrar el paradigma de paralelismo basado en directivas en Go. Este antecedente es valioso porque ilustra empíricamente la brecha semántica y de abstracción existente entre el enfoque declarativo tradicional y el modelo de concurrencia de un lenguaje moderno. Sin embargo, dado que fue desarrollado con una versión inmadura del ecosistema (limitada en sus herramientas de manipulación del Árbol de Sintaxis Abstracta y sin soporte para genéricos), no proporciona una solución técnica vigente, sino que evidencia empíricamente la alta complejidad inherente en la transformación de código. En consecuencia, la presente investigación recoge este punto de partida para superarlo mediante las capacidades actuales de transpilación de Go, incorporando el documento como evidencia complementaria y contextual al corpus principal.
...
Este formulario se aplicó a cada uno de los artículos seleccionados para la revisión sistemática. El conjunto de datos completo y detallado, producto de esta fase de extracción, se encuentra disponible para su consulta en el Anexo A.
...
- *Uso de directivas en comentarios o pragmas:* Un enfoque clásico, popularizado por OpenMP, es implementar las directivas como comentarios (ej. \/\/omp) o _pragmas_ que son ignorados por los compiladores estándar (Vikas et al., 2014; Kambites et al., 2001). Esto asegura que el código siga siendo una extensión no fundamental del lenguaje y mantenga su compilación y ejecución secuencial, facilitando la portabilidad y el desarrollo incremental (Brunschen & Brorsson, 2000).
...
- *Métricas de Rendimiento Cuantitativas:* Las métricas más fundamentales y utilizadas de forma casi universal son el tiempo de ejecución y el _speedup_ (la aceleración obtenida respecto a una versión secuencial). Prácticamente todos los estudios mencionados los utilizan como su principal indicador de rendimiento (p. ej., Piñeiro & Pichel, 2026; Perugini & Kosmidis, 2025; Kacs et al., 2024; Yoshida et al., 2017). A partir de estas, se derivan otras métricas como la eficiencia paralela (Piñeiro & Pichel, 2026; Perugini & Kosmidis, 2025) y la escalabilidad (Utting et al., 2014). En trabajos que se centran en el costo de la propia abstracción, la métrica clave es la sobrecarga (_overhead_), medida en tiempo absoluto o como un porcentaje del total (Fan et al., 2016; Guitart et al., 2001).
...
La revisión de la literatura demuestra que los lenguajes de propósito general modernos presentan limitaciones estructurales para el cómputo intensivo. Barreras técnicas como el bloqueo global del intérprete (GIL) en Python, el alto costo de los hilos en Java o la carencia de primitivas de alto nivel en Zig evidencian una clara brecha semántica y de abstracción. Esta limitación consolida el problema de los dos lenguajes, fragmentando el ecosistema al obligar a los desarrolladores a recaer en lenguajes tradicionales (como C/C++) para garantizar el rendimiento.

Para resolver esta fragmentación, los enfoques declarativos basados en directivas ofrecen la solución más eficaz mediante la separación de incumbencias, permitiendo paralelizar sin alterar la legibilidad secuencial. Aunque la implementación predominante es mediante traductores _source-to-source_, la literatura revela una alta complejidad inherente en la transformación de código. Específicamente, los enfoques basados en preprocesadores externos fracasan al carecer de un contexto semántico completo, lo que les impide gestionar construcciones modernas como la inferencia de tipos, además de sufrir caídas de rendimiento si el runtime no posee un _scheduling_ óptimo.

En conclusión, aunque la adopción de estas abstracciones eleva drásticamente la productividad y previene errores por diseño (como _deadlocks_), los desafíos de compilación documentados exigen superar los preprocesadores simples. Los trabajos futuros deben apuntar a una integración más profunda con las cadenas de herramientas de los compiladores (manipulación nativa del Árbol de Sintaxis Abstracta) y runtimes más inteligentes, paso indispensable para consolidar la viabilidad de este paradigma en ecosistemas modernos unificados.
