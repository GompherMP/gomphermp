...
Para comprender y predecir la ganancia de rendimiento potencial al paralelizar un programa, existen modelos teóricos fundamentales que establecen los límites de la aceleración (_speedup_) que se puede alcanzar.
...
- *Ley de Amdahl:*
  ...
  El _speedup_ o aceleración se define como la relación entre el tiempo de ejecución sin la mejora y el tiempo de ejecución con la mejora (Hennessy & Patterson, 2012).
  ...
  - *Aceleración de la fracción mejorada (Speedup enhanced):* Cuánto más rápida es la porción mejorada. Si un cálculo tardaba 5 segundos y ahora tarda 2, el _speedup_ de esa porción es 5/2.
...
- *Goroutines:* Son la unidad fundamental de concurrencia en Go. A diferencia de los hilos del sistema operativo, las goroutines son extremadamente ligeras y están gestionadas por el runtime; sus _stacks_ comienzan en pocos kilobytes y crecen o se reducen dinámicamente, lo que permite lanzar cientos de miles en un mismo proceso (The Go Programming Language, s. f., Effective Go).
...
- *Canales (channels):* Son conductos tipados para enviar/recibir valores entre goroutines y materializan la idea “no te comuniques compartiendo memoria; comparte memoria comunicándote” (The Go Programming Language, s. f., Effective Go). Además, el modelo de memoria de Go establece relaciones _happens-before_ para las operaciones de canal, posibilitando diseños que eviten _data races_ cuando se transfiere la propiedad de los datos entre goroutines (The Go Programming Language, 2022, The Go Memory Model).
...
- Un conjunto de directivas de compilador (conocidas como _pragmas_ en C/C++ o comentarios especiales en Fortran) que instruyen al compilador sobre cómo paralelizar una sección de código.
