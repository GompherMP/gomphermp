...
#figure(
  table(
    ...
    [HeavyReduce (taskloop)],
    [Suma de raíces cuadradas sobre un vector de $N$ elementos de punto flotante, distribuida en $P$ bloques de grano fijo mediante `taskloop`. Diseñado para estresar el planificador de tareas con cómputo intenso y alta tasa de creación de tareas.],
    [HeavyReduce (depend)],
    [Misma computación que HeavyReduce (taskloop) pero expresada como una reducción en árbol con dependencias explícitas: cuatro tareas hoja calculan sumas parciales de cuartos del vector, dos tareas intermedias combinan pares y una tarea raíz agrega el resultado. Evalúa el _overhead_ del registro de dependencias mediante cláusulas `depend(out/in)`.],
    [Sections],
...
#figure(
  table(
    ...
    [HeavyReduce (taskloop)],[2.44 ± 0.37],    [0.55 ± 0.05],   [0.99 ± 0.46],   [4.42 ± 2.44],  [2.45 ± 3.80],
    [HeavyReduce (depend)],  [2.44 ± 0.37],    [0.55 ± 0.05],   [1.38 ± 0.31],   [4.42 ± 2.44],  [1.77 ± 1.52],
  ...
  caption: [Tiempos medios e intervalos de confianza al 95% por variante a $P = 16$ ($n = 10$). HeavyReduce (depend) comparte el tiempo secuencial con HeavyReduce (taskloop) por ser el mismo algoritmo base.],
  kind: table,
...
Los resultados a $P = 16$ permiten identificar tres patrones de comportamiento diferenciados. En primer lugar, _Reduce_ constituye el resultado más destacado de la evaluación: GompherMP registra un _speedup_ de $13.35 times$ frente a los $7.66 times$ de la versión manual, diferencia cuyos intervalos de confianza no se solapan y que confirma su significancia estadística. El valor resultante ($S_{16}^"gmp" / 16 = 0.83$) supera la eficiencia paralela teórica del hardware; este comportamiento superlineal no refleja una ventaja intrínseca del runtime, sino que el patrón de acumulación privada por goroutine generado automáticamente evita la contención que introduce el patrón manual basado en canales, una interpretación que se formaliza en la @sec:amenazas. En segundo lugar, _N-Queens_ logra paridad estadística prácticamente perfecta: $9.44 times$ para GompherMP frente a $9.39 times$ para el manual, con intervalos de confianza solapados, resultado que se obtiene con una sola anotación `//gompher parallel for` sobre el bucle externo de _backtracking_. En tercer lugar, _Pipeline_ muestra una ventaja de GompherMP del 11% ($2.64 times$ vs. $2.37 times$), y _MatMul_ presenta una leve ventaja del 5% dentro del margen de error.

En el extremo opuesto, _PrefixSum_, _QuickSort_ y las variantes de _HeavyReduce_ exhiben un rendimiento inferior de GompherMP respecto al manual, con diferencias que oscilan entre el 18% (_PrefixSum_) y el 56% (_HeavyReduce_ con `task_depend`). _MergeSort_ y _Sections_ muestran _speedup_ efectivo por debajo de $1 times$ en ambas variantes paralelas: los IC solapados confirman que la diferencia entre manual y GompherMP no es estadísticamente significativa, siendo la secuencialidad del trabajo no paralelizable el factor determinante.
...
*Metodología del análisis cuantitativo.* La unidad de análisis son las funciones que implementan la lógica de paralelización en cada variante. Se excluyen los auxiliares de medición de tiempo, las funciones secuenciales, el `main` y las rutinas de utilidad compartidas entre variantes. Para los _benchmarks_ con múltiples funciones paralelas (PrefixSum, QuickSort, HeavyReduce), el conteo agrega todas las funciones que corresponden a cada variante. En cuanto a la convención de conteo dentro de cada función, se excluyen la firma de la función (incluyendo la llave de apertura), la llave de cierre final y las líneas en blanco; únicamente se contabilizan las líneas con sentencias ejecutables o directivas. Esta convención se aplica de forma uniforme a ambas variantes y es la que produce los valores $L_"man"$ y $L_"gmp"$ reportados en la @tab:tabla-loc-comparativo y en la @fig:heavyreduce-comparativo. La variación relativa de LoC se define como:
...
#figure(
  table(
    ...
    [HeavyReduce (taskloop)], [16], [21], [2],  [+31%],
    [N-Queens],             [14], [19], [2],  [+36%],
    [Pipeline],             [27], [54], [9],  [+100%],
    [HeavyReduce (depend)],   [16], [39], [8],  [+144%],
  ),
...
*GompherMP incrementa el LoC ($Delta"LoC" > 10%$).* _HeavyReduce (taskloop)_ (+31%), _N-Queens_ (+36%), _Pipeline_ (+100%) y _HeavyReduce (depend)_ (+144%) son los _benchmarks_ donde la versión GompherMP requiere más líneas que el manual. Este incremento responde a dos causas diferenciadas. En _N-Queens_ y _HeavyReduce (taskloop)_, el incremento es menor (5 a 7 líneas) y se debe a que las tareas GompherMP requieren variables auxiliares explícitas para capturar el índice de columna por tarea (e.g., `c := col`) y un arreglo de resultados indexado por posición, mientras que la versión manual puede acumular resultados directamente en un canal con semántica de cola. En _Pipeline_ y _HeavyReduce (depend)_, el incremento es estructural: expresar un grafo de dependencias de datos explícito con las cláusulas `depend(out/in)` requiere declarar variables centinela para cada arco del DAG (las variables `s0..s3` y `f0..f3` en _Pipeline_, las parciales `r0..r3`, `m0`, `m1` en _HeavyReduce (depend)_) y anotar cada tarea con sus roles de entrada y salida, elevando el LoC al doble o más respecto al manual.

*Análisis cualitativo.* Más allá del conteo de líneas, la diferencia estructural entre ambos enfoques se aprecia con mayor claridad sobre un ejemplo concreto. La @fig:heavyreduce-comparativo muestra las funciones paralelas del _benchmark_ HeavyReduce en sus dos variantes, elegido por ser representativo del caso intermedio: GompherMP introduce líneas adicionales (+31%) pero reestructura el código de forma significativa.
...
#figure(
  ...
  caption: [Comparación de implementaciones paralelas del _benchmark_ HeavyReduce. Arriba: versión manual con goroutines y canal. Abajo: versión GompherMP con directivas `taskgroup` y `taskloop`. Los dos comentarios `//gompher` son las únicas anotaciones de paralelismo.],
  kind: image,
) <fig:heavyreduce-comparativo>
...
La distinción estructural se agudiza en los extremos del espectro. En _Reduce_, el mismo patrón de distribución de trabajo se expresa con `parallel for schedule(dynamic, 64) reduction(max:m)`: una sola línea de directiva reemplaza las 24 líneas de boilerplate de la versión manual (arreglo de parciales, `sync.WaitGroup`, bucle de despacho, bucle de consolidación), dejando solo las 9 líneas del cuerpo algorítmico. En el otro extremo, _Pipeline_ y _HeavyReduce (depend)_ requieren variables centinela explícitas por cada arco del grafo de dependencias, como `s0..s3` y `f0..f3` en _Pipeline_, lo que incrementa el LoC pero deja el flujo de datos completamente auditable en el código fuente, lo que puede facilitar la revisión y el mantenimiento.
...
Finalmente, _HeavyReduce (depend)_ cierra el espectro como el único caso donde el paradigma no ofrece beneficio neto sobre el enfoque manual para las granularidades evaluadas. Su mayor incremento de LoC (+144%) y su _speedup_ más bajo ($1.77 times$) son expresión del mismo fenómeno: el _overhead_ del registro de dependencias perjudica tanto al desarrollador, que debe especificar el grafo explícitamente con variables centinela, como al runtime, que debe resolverlo con una estructura sincronizada en tiempo de ejecución. Este resultado delimita el umbral de granularidad por debajo del cual `taskloop` y `task depend` no son las herramientas adecuadas, información valiosa para guiar al usuario en la elección del patrón de directiva correcto.

En conjunto, la evaluación valida la hipótesis central del proyecto: el paradigma de paralelismo basado en directivas es técnicamente viable en el ecosistema de Go y permite cerrar la brecha semántica entre el modelo de directivas y las primitivas de concurrencia nativas del lenguaje, ofreciendo rendimiento competitivo respecto al código paralelo manual idiomático —en los patrones evaluados y sujeto a las salvedades de la @sec:amenazas— con un esfuerzo de programación significativamente menor para los patrones de paralelismo más comunes en aplicaciones Go de propósito general.


== Amenazas a la validez <sec:amenazas>

Los resultados presentados en este capítulo deben interpretarse a la luz de un conjunto de amenazas a la validez que acotan el alcance de las conclusiones.

*Validez interna.* La principal amenaza reside en la calidad de la línea base de paralelización manual, dado que las comparaciones de _speedup_ dependen directamente de qué tan idiomática y eficiente sea dicha versión. Las implementaciones manuales se desarrollaron siguiendo las mejores prácticas del ecosistema de Go (goroutines, `sync.WaitGroup`, canales y operaciones atómicas), pero no puede descartarse que un patrón manual alternativo alcanzara un rendimiento superior en casos puntuales. Esta consideración es especialmente relevante en _Reduce_, donde el _speedup_ de GompherMP excede la eficiencia paralela teórica del hardware: dicho comportamiento superlineal no debe interpretarse como una superioridad intrínseca del runtime, sino como evidencia de que el patrón manual basado en canales incurre en contención que la acumulación privada por goroutine evita. En consecuencia, las afirmaciones de ventaja de GompherMP frente al manual se enuncian como rendimiento competitivo, y no como una mejora generalizable a toda implementación manual concebible.

*Validez de constructo.* El uso de las líneas de código (LoC) como indicador cuantitativo de la expresividad es una aproximación imperfecta, sensible al estilo de formato y a las convenciones de conteo; por ello se complementó con un análisis cualitativo de la separación de incumbencias y la legibilidad. De forma análoga, la cobertura de instrucciones verifica que el código se ejecuta durante las pruebas, pero no constituye una garantía formal de equivalencia semántica entre la versión transpilada y la intención del programador; esta limitación se mitiga parcialmente mediante el detector de carreras nativo (`go test -race`) y las pruebas de extremo a extremo, aunque no se realizó una verificación exhaustiva sobre un espacio amplio de entradas.

*Validez de conclusión estadística.* Las mediciones se basan en $n = 10$ repeticiones por configuración, y los intervalos de confianza se construyeron mediante la distribución $t$-Student, que asume normalidad de los tiempos de ejecución, supuesto que puede no satisfacerse dada la asimetría habitual de estas distribuciones. Asimismo, el criterio de no solapamiento de intervalos de confianza empleado para evaluar la significancia es conservador y no equivale a una prueba de hipótesis formal.

*Validez externa.* La totalidad de las mediciones se obtuvo en una única plataforma (AMD Ryzen 7 3700X bajo Go 1.22), sin control fino sobre la frecuencia dinámica ni el _throttling_ térmico, por lo que los resultados absolutos no son directamente generalizables a otras arquitecturas. Además, la suite de diez algoritmos no agota el espacio de patrones de paralelismo, y las conclusiones se circunscriben al ecosistema de Go, sin extenderse a otros lenguajes de propósito general modernos.

Las brechas de rendimiento observadas no cuestionan la viabilidad del modelo; identifican con precisión los cuatro puntos de la implementación actual donde existe margen de mejora y configuran la agenda de desarrollo de una versión más madura de la herramienta.
