...
func sumManual(data []float64) float64 {
	...
	for t := 0; t < p; t++ {
		...
		go func(s, e int) {
			local := 0.0
			for _, v := range data[s:e] { local += v }
			ch <- local
		}(s, e)
	}
	total := 0.0
...
func phasesManual(data []int) int {
	...
	for t := 0; t < p; t++ {
		...
		if t == p-1 { e = len(data) }
		wg.Add(1)
		go func(s, e int) { defer wg.Done(); for i := s; i < e; i++ { data[i] *= 2 } }(s, e)
	}
	wg.Wait()
	data[0] = -999
	for t := 0; t < p; t++ {
		...
		if t == p-1 { e = len(data) }
		wg.Add(1)
		go func(s, e int) { defer wg.Done(); for i := s; i < e; i++ { data[i] += 1 } }(s, e)
	}
	wg.Wait()
	return data[0]
}
...
func main() {
	...
	t0 = time.Now()
	var rg float64
	for r := 0; r < runs; r++ { rg = sumGompher(data) }
	tGmp := time.Since(t0) / time.Duration(runs)

	small := make([]int, 100)
	for i := range small { small[i] = i }
	sentM := phasesManual(small)
	for i := range small { small[i] = i }
	sentG := phasesGompher(small)

	fmt.Printf("B3 PrefixSum\tseq=%v\tmanual=%v\tgompher=%v\tspeedup_manual=%.2fx\tspeedup_gompher=%.2fx\tgmp_vs_manual=%.2fx\tcorrect=%v/%v\tphases=%v/%v\n",
		tSeq, tMan, tGmp,
