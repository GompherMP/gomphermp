...
func processSeq(data []float64) (float64, float64, float64) {
	...
	for i := 0; i < n; i++ { s0 += math.Sqrt(data[i]) }
	for i := n; i < 2*n; i++ { s1 += data[i] * data[i] }
	for i := 2*n; i < 3*n; i++ { s2 += math.Log1p(data[i]) }
	return s0, s1, s2
}
...
func processManual(data []float64) (float64, float64, float64) {
	...
	wg.Add(3)
	go func() { defer wg.Done(); for i := 0; i < n; i++ { s0 += math.Sqrt(data[i]) } }()
	go func() { defer wg.Done(); for i := n; i < 2*n; i++ { s1 += data[i] * data[i] } }()
	go func() { defer wg.Done(); for i := 2*n; i < 3*n; i++ { s2 += math.Log1p(data[i]) } }()
	wg.Wait()
	return s0, s1, s2
}
...
func main() {
	...
	for i := range data { data[i] = float64(i%1000) + 1.0 }
	runs := 5

	t0 := time.Now()
	var a0s, b0s, c0s float64
	for r := 0; r < runs; r++ { a0s, b0s, c0s = processSeq(data) }
	tSeq := time.Since(t0) / time.Duration(runs)

	t0 = time.Now()
	var a0m, b0m, c0m float64
	for r := 0; r < runs; r++ { a0m, b0m, c0m = processManual(data) }
	tMan := time.Since(t0) / time.Duration(runs)

	t0 = time.Now()
	var a0g, b0g, c0g float64
	for r := 0; r < runs; r++ { a0g, b0g, c0g = processGompher(data) }
	...
	fmt.Printf("B6 Sections\tseq=%v\tmanual=%v\tgompher=%v\tspeedup_manual=%.2fx\tspeedup_gompher=%.2fx\tgmp_vs_manual=%.2fx\tcorrect=%v/%v\tcounter=%d(=%d cores)\n",
		tSeq, tMan, tGmp,
